...
from prompt import instr_eng, instr_sum
...
def process_sum(instr, text01, text02, text03, text04, text05):
    if text01=='': #is None or text02 is None or text03 is None or text04 is None or text05 is None:
        return "comment is none"
    else:
        comment = text01 + " " + text02 + " " + text03 + " " + text04 + " " + text05
        ai_sum = baidu_ernie(instr, comment)
        return ai_sum

# archived files are stored in ./result
def process_archive1(image, revise, advice=''):
    output_folder = './result'
    if image is None:
        return "No composition to be archived"
    else:
        last_dot_index = image.rfind('.')
        output_file = image[:last_dot_index] + '_arch.txt'
        ...
        if not os.path.exists(output_filepath):
            ...
            with open(output_filepath, 'w', encoding='utf-8') as output_file:
                ...
                return f'保存文件：{output_filepath}'
        else:
            return f'{output_filepath} already exists'

def process_archive2(image, revise, advice=''):
    if image is None:
        return "No composition to be archived"
    else:
        content = revise + "\n" + advice
        ...
        timestamp = str(int(timestamp_msec))

        salt = "pqcn240430mark"
        data = f"cnResult={encoded_text}&timestamp={timestamp}&uniCode=student_id" + salt
        sha256 = hashlib.sha256()
        sha256.update(data.encode())
        signature = sha256.hexdigest()

        headers = {'Content-Type': 'application/json'}
        body = {"cnResult":content, 
                "timestamp":timestamp,
                "uniCode":"student_id",
                "sign":signature}
    ...
    with gr.Tab("记叙文批改"):
        with gr.Row():
            ...
            with gr.Column():
                ...
                btn07 = gr.Button("汇总", size="sm")
                state07 = gr.State(instr_sum)
                text07 = gr.Text(label='总结')
                btn07.click(fn=process_sum, inputs=[state07,text01,text02,text03,text04,text05], outputs=[text07])
                text08 = gr.Text(label='Consol')
                btn08 = gr.Button("归档Server", size="sm")
                btn08.click(fn=process_archive1, inputs=[img01, text06, text07], outputs=[text08])
                btn09 = gr.Button("归档Pad", size="sm")
                btn09.click(fn=process_archive2, inputs=[img01,text06, text07], outputs=[text08])
    with gr.Tab("议论文批改"):
        with gr.Row():
            with gr.Column():
            ...
            with gr.Column():
                ...
                btn17 = gr.Button("汇总", size="sm")
                state17 = gr.State(instr_sum)
                text17 = gr.Text(label='总结')
                btn17.click(fn=process_sum, inputs=[state17,text11,text12,text13,text14,text15], outputs=[text17])
    with gr.Tab("批量作文改写"):

...
ernie_request_url = api_host + "rpc/2.0/ai_custom/v1/wenxinworkshop/chat/completions_pro"


def baidu_ocr(image, output_filepath):
    f = open(image, 'rb')
...
async def baidu_ernie(input, instruction, prompt):
...
# 处理单个文件的异步任务
async def process_file(input_filepath, output_filepath):
    response = await baidu_ocr(input_filepath)
    ocr_text = ''
    for line in response.json().get('words_result'):
        ocr_text += line['words']
    revised_text = await baidu_ernie(ocr_text, instruction_revise, message_revise)
...
async def main():
    ...
    for filename in os.listdir(input_folder):
        ...
        if not os.path.exists(output_filepath):
            await baidu_ocr(input_filepath, output_filepath)
        else:
